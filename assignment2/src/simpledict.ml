...
type 'a dict = Leaf | Branch of string * 'a * 'a dict * 'a dict

let rec size : 'a dict -> int =
 fun d -> match d with Leaf -> 0 | Branch (_, _, l, r) -> 1 + size l + size r
...
let is_ordered d =
  ...
  let rec helper l =
    ...
    | [ (k, _) ] -> true
    | (k1, _) :: (k2, v2) :: t ->
        if String.(k1 < k2) then helper ((k2, v2) :: t) else false
  in
  helper list
  ...
  | Branch (k, v, l, r) -> (
      ...
      | None ->
          merge
            ...
            (merge (get_unique_items d1 l) (get_unique_items d1 r))
...
let rec get_common_items d1 d2 ~merger =
  ...
  | Branch (k, v, l, r) -> (
      match lookup k d1 with
      | None ->
          merge (get_common_items d1 l ~merger) (get_common_items d1 r ~merger)
      | Some v1 ->
          merge
            (Branch (k, merger (Some v1) (Some v), Leaf, Leaf))
            (merge
               (get_common_items d1 l ~merger)
               (get_common_items d1 r ~merger)))
...
let merge_with d1 d2 ~merger =
  ...
  let single_merged1 =
    map single_in_tree1 ~f:(fun _ v -> merger (Some v) None)
  in
  let single_merged2 =
    map single_in_tree2 ~f:(fun _ v -> merger None (Some v))
  in
 ...
 fun d ~f ->
  let rec helper (d : 'a dict) : 'a dict =
    ...
    | Branch (k, v, l, r) ->
        let l' = helper l in
        let r' = helper r in
        if f k v then insert (merge l' r') k v else merge l' r'
  in
  helper d
...
(* aux functions for keywordcount.ml *)
...
let count_keywords (lst : string list) : int dict =
  ...
  let rec helper (lst : string list) (d : int dict) : int dict =
    ...
    | [] -> d
    | h :: t ->
        if is_in_list h keyword_list then
          helper t (insert d h ((lookup h d |> Option.value ~default:0) + 1))
        else helper t d
  in
  helper lst Leaf

(* sort list of string * int according to the integer value in the tuple *)
let sort (lst : (string * int) list) : (string * int) list =
  List.sort ~compare:(fun (_, a) (_, b) -> Int.compare b a) lst
...
let generate_valid_list path =
  In_channel.read_all path |> String.strip |> string_to_list
  |> remove_literal_string |> remove_comment

(** convert char list to string list by forming new strings with the characters until non-character is found *)
let rec split_chars lst acc str =
  match lst with
  | [] -> acc @ [ str ]
  | h :: t ->
      if (not (Char.is_alphanum h)) && Char.(h <> '_') then
        split_chars t (acc @ [ str ]) ""
      else split_chars t acc (str ^ String.of_char h)
