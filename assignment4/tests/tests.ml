...
let test_chunks _ = 
  ...
  assert_raises (Failure("n should be positive")) @@ (fun () -> chunks 0 [1; 2; 3])
...
let test_sample_sequence _ = 
  assert_equal [1;2] @@ NGrams.sample_sequence dist1 ~max_length: 2 ~initial_ngram: [1];
  assert_equal [1; 2;3;4] @@ NGrams.sample_sequence dist2 ~max_length: 4 ~initial_ngram: [1;2]
